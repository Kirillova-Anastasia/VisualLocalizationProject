...
class BaseDataset(data.Dataset):
    ...
    """
    def __init__(self, datasets_folder="datasets", dataset_name="pitts250k", split="train"):
        super().__init__()
        # self.args = args
...
class TripletsDataset(BaseDataset):
    ...
    """
    def __init__(self, datasets_folder="datasets", dataset_name="pitts250k", split="train", negs_num_per_query=10):
        super().__init__(datasets_folder, dataset_name, split)
        self.mining = "full"
        self.neg_samples_num = 1000  # Number of negatives to randomly sample
        ...
        self.query_transform = T.Compose([
                # T.ColorJitter(0, 0, 0, 0),
                # T.RandomPerspective(0),
                # T.RandomResizedCrop(size=self.resize, scale=(1-0, 1)),
    ...
    def compute_triplets(self, model):
        ...
        if self.mining == "full":
            self.compute_triplets_full(model)
        elif self.mining == "partial" or self.mining == "msls_weighted":
            self.compute_triplets_partial(model)
        elif self.mining == "random":
            self.compute_triplets_random(model)
    
    @staticmethod
    def compute_cache(model, subset_ds, cache_shape):
        ...
        with torch.no_grad():
            for images, indexes in tqdm(subset_dl, desc = "Compute cache"):
    ...
    def get_best_positive_index(self, query_index, cache, query_features):
    ...
    def get_hardest_negatives_indexes(self, cache, query_features, neg_samples):
    ...
    def compute_triplets_random(self, model):
        self.triplets_global_indexes = []
        # Take 1000 random queries
        sampled_queries_indexes = np.random.choice(self.queries_num, 1000, replace=False)
        ...
        cache = self.compute_cache(model, subset_ds, (len(self), features_dim))
        ...
        for query_index in tqdm(sampled_queries_indexes, desc = "Compute triplets random"):
            query_features = self.get_query_features(query_index, cache)
            best_positive_index = self.get_best_positive_index(query_index, cache, query_features)
    ...
    def compute_triplets_full(self, model):
        self.triplets_global_indexes = []
        # Take 1000 random queries
        sampled_queries_indexes = np.random.choice(self.queries_num, 1000, replace=False)
        ...
        # This loop's iterations could be done individually in the __getitem__(). This way is slower but clearer (and yields same results)
        for query_index in tqdm(sampled_queries_indexes, desc = "Compute triplets full"):
            query_features = self.get_query_features(query_index, cache)
            best_positive_index = self.get_best_positive_index(query_index, cache, query_features)
    ...
    def compute_triplets_partial(self, model):
        ...
        if self.mining == "partial":
            ...
            sampled_queries_indexes = np.random.choice(self.queries_num, 1000, replace=False, p=self.weights)
        ...
        subset_ds = Subset(self, database_indexes + list(sampled_queries_indexes + self.database_num))
        cache = self.compute_cache(model, subset_ds, cache_shape=(len(self), features_dim))
        
        # This loop's iterations could be done individually in the __getitem__(). This way is slower but clearer (and yields same results)
        for query_index in tqdm(sampled_queries_indexes, desc = 'Compute triplets partial'):
            query_features = self.get_query_features(query_index, cache)
            best_positive_index = self.get_best_positive_index(query_index, cache, query_features)
